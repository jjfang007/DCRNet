...
class SubDataset:
    def __init__(self, sub_meta, cl, transform=transforms.ToTensor(), target_transform=identity, min_size=50):
        self.sub_meta = sub_meta  # images list
        self.cl = cl
...
class SetDataset:
    def __init__(self, data_file, batch_size, transform):
        ...
        self.cl_list = np.unique(self.meta['image_labels']).tolist()  # 所有类别的列表

        self.sub_meta = {}  # {label:[image1,image2,...]} label : images belong to label
        ...
        self.sub_dataloader = []  # 每个元素为一个列表 每一个列表里放着属于同一类的batch_size张图片的loader
        for cl in self.cl_list:
